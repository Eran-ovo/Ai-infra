// ---------------------------------------------------------------------------
// v5: FlashAttention v4 + P fragment 寄存器直连
//
...
//
// Stage 3：移除 Ps shared-memory 中转，softmax 概率直接重排为 PV 的 A fragment；
// 保留 full/causal 与任意 N 支持，Bq=Bc=D=64 固定。
// ---------------------------------------------------------------------------
...
#ifndef FLASHATTENTION_V5_NO_MAIN
int main() {
    ...
    for (int N : {1, 7, 63, 64, 65, 127, 128, 129, 256, 512, 1024, 2048, 4096}) {
        ...
        for (int causal = 0; causal <= 1; ++causal) {
            ...
            printf("v5 fp16-mma-regP N=%4d mode=%-7s | %s maxErr=%.4f relErr=%.2e | %.3f ms\n",
