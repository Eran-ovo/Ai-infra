// ---------------------------------------------------------------------------
// v6: FlashAttention v5 的 D=128 实验版
//
...
//
// Stage 4：保留 register-P 路径，把 head dimension 扩展到 D=128；
// Bq=Bc=64，QK 的归约维变成 8 个 BK=16 步，PV 输出变成 16 个 n-tile。
// ---------------------------------------------------------------------------
...
#define D   128    // head dim（mma 的 N 维，也是 QK^T 的归约维）
#define Bq  64    // Q 块行数
#define Bc  64    // K/V 块行数
#define BK  16    // mma 的 K 步（D=128 = 8*16）
...
__global__ void flash_fp16_mma_v6(const __half* __restrict__ Q,
                               ...
                               __half* __restrict__ O, int N)
{
    ...
    // grid.y 将 batch 与 head 展平。单头 [N,D] 调用的 grid.y=1，offset=0，
    // 因此保留原有语义；[B,H,N,D] 则让每个 (batch, head) 独立扫描自己的 KV。
    const size_t sequence_offset =
        static_cast<size_t>(blockIdx.y) * static_cast<size_t>(N) * D;
    Q += sequence_offset;
    K += sequence_offset;
    V += sequence_offset;
    O += sequence_offset;
    ...
    float o_acc[2][16][4];   // [行 0/1][n-tile 0..15][c0..c3]
    #pragma unroll
    ...
    for (int kv = 0; kv < num_kv; ++kv) {
        ...
        for (int b0 = 0; b0 < Bc; b0 += BK) {
            // P 已经以 S fragment 的布局驻留在寄存器里，不需要先写入 Ps
            // 再从 shared memory 重载。两个连续的 8 列 S tile 正好组成
            // mma A fragment 所需的 16 列：a0/a2 属于行 g，a1/a3 属于行 g+8。
            const int st = b0 / WMMA_N;
            __half2 pa0 = __halves2half2(__float2half(S[st][0]),     __float2half(S[st][1]));
            __half2 pa1 = __halves2half2(__float2half(S[st][2]),     __float2half(S[st][3]));
            __half2 pa2 = __halves2half2(__float2half(S[st + 1][0]), __float2half(S[st + 1][1]));
            __half2 pa3 = __halves2half2(__float2half(S[st + 1][2]), __float2half(S[st + 1][3]));
            #pragma unroll
            for (int t = 0; t < 16; ++t) {
                int nd = t * WMMA_N;
...
#ifndef FLASHATTENTION_V6_NO_MAIN
int main() {
    ...
    for (int N : {1, 7, 63, 64, 65, 127, 128, 129, 256, 512, 1024, 2048, 4096}) {
        ...
        for (int causal = 0; causal <= 1; ++causal) {
            ...
            auto launch = [&]() {
                if (causal)
                    flash_fp16_mma_v6<true><<<grid, WARPS * 32>>>(dQ, dK, dV, dO, N);
                else
                    flash_fp16_mma_v6<false><<<grid, WARPS * 32>>>(dQ, dK, dV, dO, N);
            };
            ...
            printf("v6 fp16-mma-regP N=%4d mode=%-7s | %s maxErr=%.4f relErr=%.2e | %.3f ms\n",
