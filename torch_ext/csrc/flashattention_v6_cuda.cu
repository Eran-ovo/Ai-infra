// PyTorch 包装：v6 将 register-P FlashAttention 扩展到 D=128。
...
#define FLASHATTENTION_V6_NO_MAIN
#include "../../kernels/flashattention_v6.cu"
#undef FLASHATTENTION_V6_NO_MAIN

// q/k/v 支持单头 [N,128] 或批量多头 [B,H,N,128]，输出形状与输入相同。
torch::Tensor flashattention_v6_forward(torch::Tensor q,
                                        torch::Tensor k,
                                        torch::Tensor v,
                                        bool causal) {
    ...
    TORCH_CHECK(q.dim() == 2 || q.dim() == 4,
                "q/k/v must be [N, 128] or [B, H, N, 128]");
    ...
    TORCH_CHECK(q.size(-1) == D, "head dim must be 128");
    ...
    if (causal) {
        flash_fp16_mma_v6<true><<<grid, WARPS * 32, 0, stream>>>(
            reinterpret_cast<const __half*>(q.data_ptr<at::Half>()),
            reinterpret_cast<const __half*>(k.data_ptr<at::Half>()),
            reinterpret_cast<const __half*>(v.data_ptr<at::Half>()),
            reinterpret_cast<__half*>(o.data_ptr<at::Half>()), N);
    } else {
        flash_fp16_mma_v6<false><<<grid, WARPS * 32, 0, stream>>>(
            reinterpret_cast<const __half*>(q.data_ptr<at::Half>()),
            reinterpret_cast<const __half*>(k.data_ptr<at::Half>()),
            reinterpret_cast<const __half*>(v.data_ptr<at::Half>()),
            reinterpret_cast<__half*>(o.data_ptr<at::Half>()), N);
