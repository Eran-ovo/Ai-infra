// PyTorch 包装：v5 移除 v4 的 Ps shared-memory 中转，P fragment 在寄存器中直连 PV MMA。
...
// q/k/v 支持单头 [N,64] 或批量多头 [B,H,N,64]，输出形状与输入相同。
torch::Tensor flashattention_v5_forward(torch::Tensor q,
                                        torch::Tensor k,
                                        torch::Tensor v,
                                        bool causal) {
    ...
    if (causal) {
        flash_fp16_mma_v5<true><<<grid, WARPS * 32, 0, stream>>>(
            reinterpret_cast<const __half*>(q.data_ptr<at::Half>()),
            reinterpret_cast<const __half*>(k.data_ptr<at::Half>()),
            reinterpret_cast<const __half*>(v.data_ptr<at::Half>()),
            reinterpret_cast<__half*>(o.data_ptr<at::Half>()), N);
    } else {
        flash_fp16_mma_v5<false><<<grid, WARPS * 32, 0, stream>>>(
            reinterpret_cast<const __half*>(q.data_ptr<at::Half>()),
            reinterpret_cast<const __half*>(k.data_ptr<at::Half>()),
            reinterpret_cast<const __half*>(v.data_ptr<at::Half>()),
            reinterpret_cast<__half*>(o.data_ptr<at::Half>()), N);
